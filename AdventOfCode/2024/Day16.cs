...
public class Day16: IDay
{
    ...
    long GetResult(bool partA)
    {
        ...
        queue.Enqueue((SPos.row, SPos.col, Direction.Down, 1000, [(SPos.row, SPos.col)], $"({SPos.row}, {SPos.col})"),
            1000);
        queue.Enqueue((SPos.row, SPos.col, Direction.Up, 1000, [(SPos.row, SPos.col)], $"({SPos.row}, {SPos.col})"),
            1000);
        ...
        while (queue.Count > 0)
        {
            ...
            if (IsInGrid(pos1))
            {
                ...
                {
                    AddToQueue(pos1.row, pos1.col, direction, nextscore, pos.set,
                        pos.path + $"({pos1.row}, {pos1.col})");
                }
            }
            ...
            if (IsInGrid(pos2))
            {
                ...
                {
                    AddToQueue(pos2.row, pos2.col, IncrementEnum(direction), nextscore, pos.set,
                        pos.path + $"({pos2.row}, {pos2.col})");
                }
            }
            ...
            if (IsInGrid(pos3))
            {
                ...
                {
                    AddToQueue(pos3.row, pos3.col, DecrementEnum(direction), nextscore, pos.set,
                        pos.path + $"({pos3.row}, {pos3.col})");
                }
            }
        }
